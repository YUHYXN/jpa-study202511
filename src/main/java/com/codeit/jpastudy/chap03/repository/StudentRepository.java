...
//    @Query(value = "SELECT * FROM tbl_student WHERE stu_name = :n OR city = :ctiy", nativeQuery = true);

    // JPQL
    // 엔터티 클래스 기반으로 쿼리를 작성
    @Query("SELECT stu FROM Student stu WHERE stu.name = ?1 OR stu.city = ?2")
    List<Student> getStudentByNameOrCity(String name);
    List<Student> getStudentByNameOrCity2(String name);

    // 특정 이름이 포함된 학생 리스트 조회하기
    // as 는 별칭 붙이기
    // 특정 컬럼만 조회한다면 Tuple, Objct[]로도 받을 수 있음
